...
export function AppRefundPolicyPage() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    setIsLoading(true)
    // Fetch the refund policy for the current user's role
    // Type should match what was seeded: 'refund'
    apiRequest(`/policies/refund/${user.role}`)
      .then((res) => {
        if (mounted) setContent(res.data?.content || 'No refund policy available.')
      })
      .catch(() => {
        if (mounted) setError('Failed to load refund policy.')
      })
      .finally(() => {
    ...
    <div className="mx-auto max-w-3xl pt-2 pb-12">
      <div className="mb-6 px-1">
        ...
        <p className="mt-1 text-sm font-medium text-slate-500">
          Guidelines and eligibility for processing refunds.
        </p>
      </div>
